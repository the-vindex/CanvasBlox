...
export function ImportModal({ isOpen, onClose, onImport }: ImportModalProps) {
  ...
  const handleImport = () => {
    try {
      const levelData = LevelSerializer.deserialize(jsonText);
      onImport(levelData);
      onClose();
      setJsonText('');
      toast({
...
export function ExportModal({ isOpen, onClose, levelData }: ExportModalProps) {
  const jsonString = LevelSerializer.serialize(levelData);
  const { toast } = useToast();
  ...
  const handleDownloadJSON = () => {
    LevelSerializer.downloadJSON(levelData);
    toast({
      title: "Downloaded",
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted/30 border border-border rounded p-3">
            ...
            <ul className="text-xs text-muted-foreground space-y-1">
              <li><strong>metadata:</strong> Level information (name, dimensions, author, creation date)</li>
              <li><strong>tiles:</strong> Array of platform tiles with position, dimensions, rotation, and material properties</li>
              <li><strong>objects:</strong> Interactable elements (buttons, doors, levers) with linking system for game logic</li>
              <li><strong>spawnPoints:</strong> Player and enemy spawn locations with facing direction and spawn IDs</li>
              <li><strong>Linking:</strong> Objects reference each other via ID arrays (linkedObjects, linkedFrom)</li>
              <li><strong>Rotation:</strong> All rotations in degrees (0, 90, 180, 270)</li>
              <li><strong>Layers:</strong> Z-index for rendering order (0 = background, higher = foreground)</li>
            </ul>
          </div>
        </div>
