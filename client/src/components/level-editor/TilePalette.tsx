...
function TileItem({ type, label, color, icon, isSelected, onSelect, usePlatformTexture }: TileItemProps) {
    ...
    return (
        <button
            type="button"
            className={cn(
                'tile-item bg-secondary/60 backdrop-blur-sm border border-white/10 rounded p-2 text-center transition-all duration-150 hover:transform hover:-translate-y-0.5 shadow-lg hover:shadow-xl cursor-default',
        ...
        >
            <div
                className={cn(
                    'w-full h-12 rounded mb-1 flex items-center justify-center overflow-hidden',
                    !usePlatformTexture && color
                )}
            >
                {usePlatformTexture ? (
