...
test.describe('Linking Tool', () => {
    ...
    test('should show visual selection around link source', async ({ page }) => {
        const buttonTile = page.getByTestId('tile-button');
        const linkTool = page.getByTestId('tool-link');
        ...
        // Selection count should show 1 (button is selected)
        const selectionCount = page.getByTestId('selection-count');
        await expect(selectionCount).toContainText('1');
    });

    test('should clear selection when deselecting link source', async ({ page }) => {
        const buttonTile = page.getByTestId('tile-button');
        const linkTool = page.getByTestId('tool-link');
        ...
        // Click button as source (should select it)
        await clickCanvas(page, 200, 200);
        await page.waitForTimeout(100);
    ...
    test('should clear selection after creating link', async ({ page }) => {
        const buttonTile = page.getByTestId('tile-button');
        const doorTile = page.getByTestId('tile-door');
        const linkTool = page.getByTestId('tool-link');
        ...
        // Click door to create link
        await clickCanvas(page, 400, 200);
        await page.waitForTimeout(100);

        // Selection should be cleared after link creation
        await expect(selectionCount).toContainText('0');
    });
});
